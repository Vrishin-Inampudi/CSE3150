...
def labGen():
    ...
    #ensures that 'SkyRedFalcon914' is the first member and randomize student's gang members
    random.shuffle(gang)
    if 'SkyRedFalcon914' in gang:
        gang.remove('SkyRedFalcon914')
    gang.append('SkyRedFalcon914')
    gang.reverse()   

    #Q1: A most common password for SkyRedFalcon914
    ...
    #iterate and add solutions to dictionary. The first 6 random gang members (including SkyRedFalcon914) are chosen as the solution. 
    #add remainder of the gang members with a random complex password that is hard to guess.
    studentSolution[f"Q1"] = f"{pwd[0]}" #SkyRedFalcon914's password
    for i in range(1,6):
